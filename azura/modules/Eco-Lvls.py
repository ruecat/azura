...
class ecolvls(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        user_experience = int(redis_client.get(experience_key.format(message.author)) or 0)
        user_money = int(redis_client.get(money_key.format(message.author)) or 0)
        ...
        redis_client.set(experience_key.format(message.author), user_experience)
        redis_client.set(money_key.format(message.author), user_money)

        user_level = int(redis_client.get(level_key.format(message.author)) or 1)
        ...
        redis_client.set(experience_key.format(message.author), user_experience)
        redis_client.set(level_key.format(message.author), user_level)
    ...
    async def profile(self, interaction: nextcord.Interaction):
        ...
        user_money = int(redis_client.get(money_key.format(interaction.user)) or 0)
        user_level = int(redis_client.get(level_key.format(interaction.user)) or 1)

        profile = nextcord.Embed(title=f"Айве, {interaction.user.name}!", color=0x3d81ff)
        profile.set_thumbnail(url=interaction.user.avatar.url)
        profile.set_footer(text=f"ID: {interaction.user.id}")
        profile.add_field(name="Trust rank:", value=f"🟧 Unknown", inline=False)
    ...
    async def Profile(self, interaction: nextcord.Interaction, member: nextcord.Member):
        ...
        user_experience = int(redis_client.get(experience_key.format(member)) or 1)
        user_money = int(redis_client.get(money_key.format(member)) or 0)
        user_level = int(redis_client.get(level_key.format(member)) or 1)

        profile = nextcord.Embed(title=f"Айве, {member.name}!", color=0x3d81ff)
